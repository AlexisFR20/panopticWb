...
def admin_tipo_movimiento_listado(request): 
    userdict    = user_validation.validate(request)
    user          = User.objects.get(pk= userdict.get('user_id') )
    all_users   = User.objects.all()
    #l = request.user.groups.values_list('name',flat = True) # QuerySet Object
    #l_as_list = list(l)       
    return render(request, 'administrador/ddmanagement/tipo_movimiento/index.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'user': user,  'all_users': all_users })

def admin_tipo_movimiento_create(request): 
    userdict    = user_validation.validate(request)
    user          = User.objects.get(pk= userdict.get('user_id') )
    all_users   = User.objects.all()
    #l = request.user.groups.values_list('name',flat = True) # QuerySet Object
    #l_as_list = list(l)       
    return render(request, 'administrador/ddmanagement/tipo_movimiento/create.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'user': user,  'all_users': all_users })

def admin_tipo_movimiento_edit(request, mov_id): 
    userdict    = user_validation.validate(request)
    user          = User.objects.get(pk= userdict.get('user_id') )
    all_users   = User.objects.all()
    #l = request.user.groups.values_list('name',flat = True) # QuerySet Object
    #l_as_list = list(l)       
    return render(request, 'administrador/ddmanagement/tipo_movimiento/edit.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'user': user,  'all_users': all_users })
