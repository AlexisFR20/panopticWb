...
# Registro de Paqueteria - Lista
def admin_area_restringida(request):
    ...
    return render(request, 'administrador/predictive/area_restringida_lista.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'user': user, 'estrailers': estrailers })

# Registro de Paqueteria - Crear
def admin_area_restringida_registrar(request):
	userdict    = user_validation.validate(request)
	user          = User.objects.get(pk= userdict.get('user_id'))
	estrailers = F_ES_Trailers.objects.all()
	
	return render(request, 'administrador/predictive/area_restringida_registrar.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'user': user, 'estrailers': estrailers })