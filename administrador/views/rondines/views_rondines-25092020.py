...
def admin_rondines_rondin_create(request): 
    ...
    if request.method == 'POST':
        ...
        try:   
            ...
            go = None        
            print(e)
            messages.add_message(request,  messages.ERROR, 'No se creó el rondín, hubo un error.')
...
def ajax_agregar_mod_punto(request):
    ...
    try:
        ...
        if punto_id != '':        
            ...
            punto.save(update_fields=['lat', 'lng', 'nombre', 'activo'])
            vreturn = 'actualizado'                  
        
        else:           
            
            rondin = Rondin.objects.get(pk=punto_rondin_id)            
            punto = Punto.objects.create(orden=1, lat=punto_lat, lng=punto_lng, nombre=punto_nombre, activo=True, rondin=rondin, tareas="Sin órdenes aún")
            punto.save        
            vreturn = 'creado'                    
...
def admin_rondines_show(request, rondin_id):
    ...
    return render(request, 'administrador/rondines/show.html', { 'css_list': css_list, 'js_list': js_list, 'userdict': userdict, 'rondin': rondinToShow })
