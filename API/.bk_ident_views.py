...
def api_fes_trilers(request):
        ...
        cajon = Cajon.objects.get(pk=request.POST.get("cajon"))
...
def api_fes_trilers_salida(request):
	...
	cajon = Cajon.objects.get(trailer_id=festrailers.id)
...
def api_reportar_incidente(request):
	...
	if serializer.is_valid():
		...
		try:
			...
			notifications.sendNotification("Incidente de servicio", "Se dio de alta un incidente de servicio en unidad de negocio: "+planta.nombre, request.user.notification_token, request.user.email, noti)
