...
def rondines_dashboard(request):
    ...
    if rondines:
        rondineshechos = RondinHecho.objects.filter(reduce(lambda x, y: x | y, [Q(rondin_id=item.id) for item in rondines])).order_by('-id')
    else:
        rondines = None
        rondineshechos = None
...
def registro_visitantes(request):
    if request.method == 'POST':
        try:
            if request.POST['nombre'] is "" or request.POST['anfitrion'] is "":
                raise e()
            else:
                prev = F_Visitantes.objects.create(
                    tipo = request.POST['tipo'],
                    nombre = request.POST['nombre'],
                    empresa = request.POST['empresa'],
                    anfitrion = request.POST['anfitrion'],
                    identificacion = request.POST['identificacion'],
                    confirmo_anfitrion = "1",
                ...
                if request.POST['marca'] != "" or request.POST['modelo'] != "" or request.POST['placas'] != "" or request.POST['nota'] != "":
                ...
                messages.add_message(request,  messages.SUCCESS, 'Visitante creado exitosamente')
            
        except Exception as e:
            messages.add_message(request, messages.ERROR, 'Hubo un error al registrar el Visitante')
        return redirect("analytics:registro_visitantes")
    else:
        return render(request, "analytics/registro_visitantes.html", {"css_list": css_list, "js_list": js_list})


def registro_previsitantes(request):
    if request.method == 'POST':
        try:
            if request.POST['nombre'] is "" or request.POST['anfitrion'] is "":
                raise e()
                ...
                prev = F_Visitantes.objects.create(
                    ...
                    empresa = request.POST['empresa'],
                    anfitrion = request.POST['anfitrion'],
                    fecha_agendada = request.POST['fecha'],
                    identificacion = request.POST['identificacion'],
                    confirmo_anfitrion = "1",
                    pre = "1",
                    planta_id = request.user.planta_id
                ...
                prev.save()
                qr_u = "https://chart.apis.google.com/chart?cht=qr&chs=230x230&chl=" + str(prev.id) + "|" + str(prev.nombre) + "|" + str(prev.anfitrion) + "|" + str(prev.planta_id) + "|" + str(prev.fecha_agendada)
                F_Visitantes.objects.filter(id=prev.id).update(qr_url=qr_u)
                ...
                messages.add_message(request,  messages.SUCCESS, 'Previsitante creado exitosamente')
            
        except Exception as e:
            messages.add_message(request, messages.ERROR, 'Hubo un error al registrar el previsitante')
        return redirect("analytics:registro_previsitantes")
    else:
        return render(request, "analytics/registro_previsitantes.html", {"css_list": css_list, "js_list": js_list})

def previsitantes(request):
    planta = request.user.planta
    pre_v = F_Visitantes.objects.filter(planta_id=planta).filter(pre=1)

    return render(request, "analytics/previsitantes-historial.html", {"css_list": css_list, "js_list": js_list, "previsitantes": pre_v})
    #return render(request, "analytics/registro_previsitantes.html", {"css_list": css_list, "js_list": js_list})

def rev18puntos(request):
    cliente  = request.user.cliente    
    clientes = Cliente.objects.all()
    cualP    = request.user.planta
    planta   = Planta.objects.filter(cliente=cliente)    
    plantas  = Planta.objects.all()    
    
    revisiones = F_18_puntos_Trailer.objects.filter(planta__cliente=cliente).order_by('-fecha')  
    
    return render(request, "analytics/rev18puntos.html", {"css_list": css_list, "js_list": js_list, 'clientes': clientes, 'plantas': plantas, 'revisiones': revisiones})

def f18puntos_historial_show(request, rev_id):
    revision18 = F_18_puntos_Trailer.objects.get(pk=rev_id)        
  
    return render(request, 'analytics/rev18puntos-historial-show.html', { 'css_list': css_list, 'js_list': js_list, 'revision': revision18 })
    
class CapacitacionesListView(ListView):
    model = Concentracion    
    template_name = 'analytics/capacitacion.html'       

    def get_queryset(self):
        queryCliente = self.request.user.cliente        
        return Concentracion.objects.filter(cliente=queryCliente)
        #return Concentracion.objects.all()
...
class CapacitacionDetailView(DetailView):
    model = Concentracion    
    template_name = 'analytics/capacitacion-ver.html' 
...
def fvisitantes_dashboard(request):
    ...
    if request.user.getRolU() == "administrador":
        ...
        visitantes = F_Visitantes.objects.filter(reduce(lambda x, y: x | y, [Q(planta_id=p.id) for p in plantas])).filter(pre=0).order_by('-id')
    else:
        plantas = Planta.objects.filter(id=request.user.planta_id).first()
        visitantes = F_Visitantes.objects.filter(planta_id=request.user.planta_id).filter(pre=0).order_by('-id')
    #userdict = user_validation.validate(request)

    #visitantes = F_Visitantes.objects.get()
    #for p in plantas:
...
def controlp_dashboard(request):
    patio = Patio.objects.filter(planta_id=request.user.planta_id).first()
    if patio:
        cajones = Cajon.objects.filter(patio_id=patio.id)
        for i in cajones:
            if i.disponibilidad == 0:
                i.disponibilidad = "Ocupado"
            elif i.disponibilidad == 1:
                i.disponibilidad = "O/Caja vacía"
            else:
                i.disponibilidad = "Disponible"
                i.tipo_caja = "-"
    else:
        cajones = None
    return render(request, "analytics/controlp_dashboard.html", {"css_list": css_list, "js_list": js_list, 'patio':patio, 'cajones':cajones})
...
def es_trailers_dashboard(request):
    queryCliente = request.user.cliente        
    estrailers = F_ES_Trailers.objects.filter(planta__cliente=queryCliente)    
    
    return render(request, 'analytics/es-trailers-historial.html', { 'css_list': css_list, 'js_list': js_list, 'estrailers': estrailers })
